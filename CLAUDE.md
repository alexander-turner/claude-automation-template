...
- **CI that “runs your real config” does not exercise branches guarded out of the CI environment** (e.g. `command -v` checks, OS detection, env-var presence). Catch this two ways: (1) a static invariant over the source that holds regardless of environment; (2) an end-to-end job that reconstructs the precondition so the branch runs for real.
