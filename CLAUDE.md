...
- **When adding a persistence/cache layer, audit every skip/fast-path that accepts existing state by _presence_ rather than _integrity/version_.** A presence-only fast-path reached on verify-failure silently defeats the integrity guarantee the persistence is assumed to rest on, letting content from one input-set survive into a later trust context.
...
- **When externalizing a GitHub Actions inline `run:` block, add `set -e -o pipefail` explicitly.** GitHub invokes inline bash as `bash -eo pipefail {0}`, but a standalone script gets only what it declares. A block whose `set` line omitted `-e` or `pipefail` must have it added back, or error/pipe-failure handling silently weakens. Audit every extracted script.
- **`cmd | tee log` masks a failing `cmd`** (tee exits 0). GitHub Actions’ default `run:` shell includes `pipefail`, but action-provided `runCmd:` (devcontainers/ci) and explicit `shell: sh` bypass it. Add `set -o pipefail` in those contexts.
...
- **Per-launch hooks must be idempotent and fast.** Guard installs on presence (`command -v`); install only what’s missing; background anything heavy. An unconditional multi-package install on every launch adds dead time to every session and causes initial prompts to be processed only after the user’s first keystrokes.
...
- **Coverage % is the wrong instrument for an integration boundary.** Line coverage can hit 100% while the load-bearing invariants (what actually happens at the boundary) are entirely unobserved. Add runtime e2e tests that assert on the real stack; a silently non-engaged integration layer should be loud.
- **A persistence test that constructs assumed on-disk state rather than driving the tool to produce it stays green forever**—even after the tool moves where it stores that state. Where correctness depends on “external tool writes X where we read it,” make the tool write X and discover the location (fail loud if nothing did); don’t encode the location as a fixture. The same applies to a test that string-matches a script’s source: it asserts nothing about the script’s behavior.
...
- **Test file-writers against the full shape of the destination’s filesystem**: regular file / symlink→target / dangling symlink / symlinked parent / missing. Line coverage and mutation testing both pass on the regular-file case alone. The same matrix applies to install _and_ uninstall paths—two operations touching the same file through different primitives disagree on symlink behavior.
- **When a crash in a code path causes a security control to fail open, fuzz that path for “never crashes,” not just the adversarial-output surface.** A point-fix for one malformed shape misses sibling shapes; a generative never-crash test catches the whole class.
- **CI that “runs your real config” does not exercise branches guarded out of the CI environment** (e.g., `is_root`, `command -v`, `uname`, env var presence). Catch this two ways: (1) a static invariant over the source that holds regardless of environment; (2) an end-to-end job that reconstructs the precondition so the branch runs for real.
- **A stub that rubber-stamps one command can make a different, overloaded use of that command return a wrong verdict or spin to a timeout.** Branch stubs on the argument shape each caller uses, not just on the command name.
- **Installer/bootstrap scripts tested only on one OS leave cross-OS divergence invisible.** Add a CI matrix over every target OS, and test each written path through hostile pre-states (especially a dangling symlink)—not just the happy path.
